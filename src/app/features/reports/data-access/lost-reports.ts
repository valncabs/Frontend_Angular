...
export class LostReportsService {
  ...
  markAsFound(reportId: string): Observable<ApiSuccessResponse<LostReportResponse>> {
    // Solicitud del dueño: el admin aprueba el cambio a encontrada.
  ...
  adminMarkFound(reportId: string): Observable<ApiSuccessResponse<LostReportResponse>> {
    return this.http
      .post<ApiSuccessResponse<LostReportResponse>>(
        `${this.baseUrl}/${reportId}/admin-mark-found`,
        {},
      )
      .pipe(map((res) => ({ ...res, data: normalizeLostReportResponse(res.data) })));
  }
