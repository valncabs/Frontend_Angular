...
export class AddPetModalComponent implements OnInit, OnChanges {
  ...
  /** Razas de la especie actualmente seleccionada. Se piden al backend
   * en el momento (no se precargan todas), evitando cargar catálogos
   * completos que el usuario nunca va a usar. */
  readonly breeds = signal<BreedResponse[]>([]);
  readonly breedsLoading = signal(false);

  photoPreview = signal<string | null>(null);
  photoError = '';
  private selectedPhoto: File | null = null;

  /** Visor a pantalla completa para la foto precargada en add/editar. */
  ...
  get breedOptions(): SelectOption[] {
    return this.breeds().map((b) => ({ value: b.id, label: b.name }));
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['isOpen'] && this.isOpen) {
      ...
      if (this.petToEdit) {
        ...
        this.loadBreeds(this.petToEdit.speciesId);
      } else {
        this.breeds.set([]);
        this.form.reset({
          species_id: '',
  ...
  private loadBreeds(speciesId: string): void {
    ...
    this.catalogService.listBreeds(speciesId).subscribe({
      next: (response) => {
